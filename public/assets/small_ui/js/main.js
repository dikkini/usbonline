		...
		var template =
			_.template(
				'<div data-loader-id="<%= loaderId %>" class="loader-item trian box-shadow">' +
					'<span ' + ((ALL_LOADERS_COUNT == 1) ? 'id="lar"' : '') + ' data-loader-id="<%= loaderId %>" title="Delete or cancel burning" class="loader-action-remcancel glyphicon glyphicon-remove" aria-hidden="true">' +
					'</span>' +
					'<h4 ' + ((ALL_LOADERS_COUNT == 1) ? 'id="lsv"' : '') + ' data-loader-id="<%= loaderId %>" class="loader-status-value" data-code="0"> Waiting... </h4>' +
					'<div class="loader-interface">' +
					'<select ' + ((ALL_LOADERS_COUNT == 1) ? 'id="lts"' : '') + ' data-loader-id="<%= loaderId %>" class="loader-type-select form-control input-sm">' +
					...
					'</select>' +
					'<button ' + ((ALL_LOADERS_COUNT == 1) ? 'id="lac"' : '') + ' data-loader-id="<%= loaderId %>" title="Choose ISO file from filesystem" type="button" class="loader-action-chooseiso btn btn-default">' +
						'<span class="glyphicon glyphicon-search" aria-hidden="true"></span>' +
					'</button>' +
					'<button ' + ((ALL_LOADERS_COUNT == 1) ? 'id="lal"' : '') + ' data-loader-id="<%= loaderId %>" title="Open site of chosen loader in a new tab" type="button" class="loader-action-linktoiso btn btn-default">' +
						'<span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>' +
					'</button>' +
					'</div>' +
					'<div ' + ((ALL_LOADERS_COUNT == 1) ? 'id="linf"' : '') + ' data-loader-id="<%= loaderId %>" class="loader-information"></div>' +
					'<div data-loader-id="<%= loaderId %>" class="progress">' +
						'<div ' + ((ALL_LOADERS_COUNT == 1) ? 'id="burnprogr"' : '') + ' data-loader-id="<%= loaderId %>" class="progress-bar burning-progress" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%;">' +
							'0%' +
						'</div>' +
					'</div>' +
				'</div>');
